...
class Main(object):
    ...
    def updateBuffer(self, doc, word, url, maxnum):
        if not doc:
            return doc
        return re.sub(
            r"(" + word + ")", '[' + word + '](' + url + ')', doc, count=maxnum
        )

    @neovim.command('FeelingLuckyClear', nargs='*')
    def luckyClear(self, args):
        self.luckyStay.clear()
    ...
    def luckyEveryWhere(self, args):
        ...
        for l in self.vim.current.buffer:
            for word in d:
                line = self.updateBuffer(l, word, d[word], 1)
            if line != self.vim.current.buffer[i]:
